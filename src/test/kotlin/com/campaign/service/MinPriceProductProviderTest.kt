...
import com.campaign.service.ProductService.ProductDetail
...
class ProductServiceTest(
    ...
    fun `모든 카테고리 별 최저가격의 상품과 상품의 브랜드명을 포함한 테이블을 반환한다`() {
        ...
        val result = sut.getMinPriceProductTable()
    ...
    @Test
    fun `최저가인 상품이 복수개 존재하는 상품은 브랜드 이름이 사전순으로 앞선 상품이 선택된다`() {
        val brandA = saveBrand(brandName = "A")
        val brandABC = saveBrand(brandName = "ABC")
        val brandZ = saveBrand(brandName = "Z")
        val commonPrice = 100L

        val expectSelected = saveProduct(Product.ProductCategory.TOP, commonPrice, brandA)
        saveProduct(Product.ProductCategory.TOP, commonPrice, brandABC)
        saveProduct(Product.ProductCategory.TOP, commonPrice, brandZ)
